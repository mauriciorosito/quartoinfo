...
<form action="content.form.php" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="idContent" value="<?php if(isset($_GET["idContent"])) { echo $_GET["idContent"]; }?>">
	<table style="width:100%;padding:10px;">
		<tr>
			<td>Título: <input type="text" name="title" class="form-control" value="<?php if(isset($content) && $content->getTitle()!="") { echo $content->getTitle(); }?>"><br></td>
			<td>Fonte: <input type="text" name="source" class="form-control" value="<?php if(isset($content) && $content->getSource()!="") { echo $content->getSource(); }?>"><br></td>
		</tr>
		<tr>
			<td>Descrição: <input type="text" name="description" class="form-control" value="<?php if(isset($content) && $content->getDescription()!="") { echo $content->getDescription(); }?>"><br></td>
			<td>Tipo do conteúdo: <select name="type" class="form-control">
				<option  value="N" <?php if(isset($content) && $content->getType()== "N") { echo "selected"; } ?>> Notícia</option>
				<option value="E" <?php if(isset($content) && $content->getType()== "E") { echo "selected"; } ?>> Eventos</option>
				<option value="O" <?php if(isset($content) && $content->getType()== "O") { echo "selected"; } ?>> Oportunidades</option>
				</select>
			</td>
		</tr>
		<tr>
			<td>Visualizar desde:<input type="text" name="postDate" class="form-control" pattern="[0-3]{1}[0-9]{1}/[0-1]{1}[0-9]{1}/[2]{1}[0-9]{3}" data-mask="99/99/9999" placeholder="Data"  value="<?php if(isset($content) && $content->getPostDate()!="") { echo $content->getPostDate(); }?>"><br></td>
			<td>Visualizar até: <input type="text" name="expirationDate" class="form-control" pattern="[0-3]{1}[0-9]{1}/[0-1]{1}[0-9]{1}/[2]{1}[0-9]{3}" data-mask="99/99/9999" placeholder="Data" value="<?php if(isset($content) && $content->getExpirationDate()!="") { echo $content->getExpirationDate(); }?>"><br></td>			
		</tr>
		<tr>
			<td colspan=2>Texto: <textarea name="text" value="" class="textarea form-control" placeholder="Enter text ..." style="width: 810px; height: 200px"> <?php if(isset($content) && $content->getText()!="") { echo $content->getText(); }?> </textarea><br></td>
		</tr>
		<tr>
	...
	<input type="submit" name="button" value="<?php if(isset($_GET["action"])) { echo ucwords($_GET["action"]); }?>"><br>
</form>
